		...
		<main>
			...
			>
				<FormFieldComponent
					...
					register={register}
					errors={errors}
					placeholder="e.g. Yard sale Saturday, or Building BBQ next week"
				...
				<FormFieldComponent
					...
					register={register}
					errors={errors}
					placeholder="Events, items for sale, questions — anything neighbors should know"
					isTextArea
				/>
				<label htmlFor="tags" className="h4-semibold dark:text-babyPowder">
					Topics (press Enter to add — e.g. event, for-sale, question)
				</label>
				<Controller
					name="tags"
					control={control}
				...
				<Button
					...
					disabled={isLoading}
				>
					{isLoading ? <Spinner size="sm" /> : `Post to the board`}
				</Button>
			</form>
		</main>
	);
