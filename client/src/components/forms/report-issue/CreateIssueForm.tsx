...
export default function CreateIssueForm() {
	...
	const onSubmit = async (formValues: TIssueCreateSchema) => {
		if (!apartment?.id) {
			toast.error(
				"Add your unit in your profile first, then you can submit a maintenance request.",
			);
			return;
		}
		...
		try {
			...
			toast.success(
				"Request submitted — we'll notify building staff and email you a confirmation.",
			);
			reset();
			router.push("/profile");
		} catch (error) {
		...
		<main>
			...
			>
				...
				<FormFieldComponent
					...
					register={register}
					errors={errors}
					placeholder="Describe what's happening so we can send the right help"
					isTextArea
					startIcon={<FlagIcon className="dark:text-babyPowder size-8" />}
				/>
				<div>
							...
							<Controller
								...
								render={({ field: { onChange, onBlur, value } }) => (
									<Select
										...
										onBlur={onBlur}
										placeholder="Usually 'Reported' for a new request"
