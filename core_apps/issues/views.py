...
from .emails import send_issue_confirmation_email
from .models import Issue
from .permissions import (
    user_can_delete_issue,
    user_can_update_issue,
    user_can_view_issue,
)
...
class IssueDetailAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        if not user_can_view_issue(self.request.user, issue):
...
class IssueUpdateAPIView(generics.UpdateAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        if not user_can_update_issue(user, issue):
            logger.warning(
...
class IssueDeleteAPIView(generics.DestroyAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        user = self.request.user
        if not user_can_delete_issue(user, issue):
            logger.warning(
