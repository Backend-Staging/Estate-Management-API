...
class Post(TimeStampedModel):
    ...
    def save(self, *args, **kwargs) -> None:
        author = self.author
        if author.is_superuser or author.is_staff:
            super().save(*args, **kwargs)
            return

        profile = getattr(author, "profile", None)
        if profile and profile.role in (Profile.Role.TENANT, Profile.Role.AGENT):
            super().save(*args, **kwargs)
            return

        raise ValueError(
            "Only tenants, agents, superusers, or staff members can create posts."
        )
